...
| "Save the session state" | Update Ziggy Session State note — date, work done, decisions, files changed, open items, next actions (under 500 words) |
...
1. **Git** — Auto-commits every 10 min. Safety net. You don't manage this.
2. **Session State** — A note you update at end of each session. This is your short-term memory.
3. **Save States** — Permanent conversation records. Long-term memory.

### Session Rules
- When Nathan says "save the session state": Update the Ziggy Session State note with what was worked on, decisions made, files changed, open items, next actions. Keep under 500 words.
- When Nathan says "save this as a save state": Create a permanent note with `ziggy` + `save-state` tags.
- At session start: Read the Session State for continuity. Don't re-explain captured context.
