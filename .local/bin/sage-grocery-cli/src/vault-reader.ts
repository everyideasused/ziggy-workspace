...
import { FrontmatterData, ParsedNote } from "./types";
...
export class VaultReader {
   ...
   */
  readNote(noteName: string): ParsedNote {
   ...
   */
  private findNoteFile(noteName: string): string | null {
    try {
      ...
      for (const file of files) {
        if (
          file.toLowerCase().includes(normalizedSearch) &&
          file.endsWith(".md")
        ) {
          return path.join(VAULT_PATH, file);
        }
   ...
   */
  private parseNote(content: string, filename: string): ParsedNote {
   ...
   */
  private parseFrontmatter(yaml: string): FrontmatterData {
