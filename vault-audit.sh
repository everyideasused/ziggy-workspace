...
check_file() {
    ...
    # Read first 20 lines (enough for frontmatter + nav header)
    local content=$(head -n 20 "$file")
    ...
    # Check for navigation header (should be first non-frontmatter line)
    local first_line_after_fm=$(awk '/^---$/{p++} p==2{getline; print; exit}' "$file")
    if [[ ! "$first_line_after_fm" =~ ^\>.*🏠.*📅 ]]; then
